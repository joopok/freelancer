...
export default function TechNewsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
      <div className="mb-10">
        <h1 className="text-3xl font-bold mb-2 text-gray-900 dark:text-white">Tech News</h1>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
              activeCategory === category.id
                ...
                : 'bg-gray-100 dark:bg-gray-700 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600'
            }`}
          >
            {category.label}
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredNews.map((news) => (
          <motion.div
            ...
            className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden hover:shadow-md dark:hover:shadow-gray-900/50 transition-shadow bg-white dark:bg-gray-800 transition-colors duration-300"
          >
            <Link href={`/blog/tech-news/${news.category.toLowerCase().replace(' ', '-')}/${news.slug}`}>
              <div className="relative h-48">
                ...
                />
                <div className="absolute top-2 left-2 bg-blue-600 dark:bg-blue-700 text-white text-xs px-2 py-1 rounded">
                  {news.category}
                </div>
              </div>
              <div className="p-4">
      ...
      {filteredNews.length === 0 && (
        <div className="text-center py-10">
          <p className="text-gray-500 dark:text-gray-400">선택한 카테고리에 뉴스가 없습니다.</p>
        </div>
      )}
    </div>
  );
